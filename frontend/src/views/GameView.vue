...
const onRestartChangeTheme = async () => {
  ...
  try {
    room.value = await restartRoom(roomCode, {
      ...
      themeText: restartThemeText.value,
    });
    router.push(`/room/${roomCode}`);
  } catch (error: any) {
...
const onPhaseTimeout = async () => {
  if (!room.value || room.value.status !== 'playing' || isAutoAdvancing.value) return;
  isAutoAdvancing.value = true;
  try {
    if ((room.value.phase ?? 'action') === 'action') {
      room.value = await advanceTurn(roomCode, { playerId: currentPlayer.value?.id, reason: 'timeout' });
    } else if (room.value.phase === 'answering') {
      room.value = await advancePhase(roomCode, { playerId: currentPlayer.value?.id, reason: 'answer_timeout' });
    } else if (room.value.phase === 'result') {
      room.value = await advancePhase(roomCode, { playerId: currentPlayer.value?.id, reason: 'result_timeout' });
    }
  } catch (error: any) {
...
onMounted(async () => {
  try {
    ...
    socket.on('gameUpdated', (updatedRoom: Room) => {
      ...
      restartThemeText.value = updatedRoom.themeText;
    });
  ...
  <main class="page">
    <div class="app-shell">
      <header class="app-header">
        ...
        <button class="header-button" type="button" @click="router.push(`/room/${roomCode}`)">ルームに戻る</button>
      </header>
        ...
        <div class="member-panel">
          ...
          <ul class="player-list">
            <li v-for="player in topicDisplayPlayers" :key="player.id" class="player-item topic-player-item">
              ...
              <div style="flex: 1;">
                ...
                <div class="topic-chip" :class="{ hidden: player.isMine }">{{ player.visibleTopic }}</div>
              </div>
          ...
          <div v-if="room?.phase === 'answering' && activeQuestion" class="answer-status-box">
            ...
            <div v-for="player in answerTargetPlayers" :key="player.id" class="answer-status-row">
              ...
              <strong>{{ activeQuestion.answers.some((answer) => answer.playerId === player.id) ? '回答済' : '未回答' }}</strong>
            </div>
          </div>
        </div>
        ...
        <div class="game-panel">
          <div class="topic-card">
            <div>
              ...
              <p v-if="currentPlayer && room?.status === 'playing'">現在の手番：{{ currentPlayer.playerName }} さん</p>
              <p>状態：{{ phaseLabel }}</p>
            </div>
          </div>

          <div v-if="room?.status === 'finished' || room?.status === 'closed'" class="winner-box">
            <template v-if="room?.winnerName">
              <h2>🎉 {{ room.winnerName }} さんの勝利！</h2>
              <p>正解：{{ room.correctTopic }}</p>
            </template>
            <template v-else>
              <h2>{{ room?.status === 'closed' ? '部屋を閉じました' : 'ゲーム終了' }}</h2>
              <p>ターン上限に達しました。</p>
            </template>
          ...
          <div v-if="(room?.phase ?? 'action') === 'action' && room?.status === 'playing'" class="turn-action-box">
            <div class="action-tabs">
              <button type="button" :class="{ active: actionType === 'question' }" @click="actionType = 'question'">質問する</button>
          ...
          <div v-if="room?.phase === 'answering' && activeQuestion" class="question-phase-box">
            ...
            <template v-if="amIQuestioner"><div class="hint-box">他プレイヤーの回答を待っています。</div></template>
          ...
          <div class="log-panel">
            ...
            <div v-for="log in room?.gameLogs ?? []" :key="log.id" class="log-item">
              <strong>{{ log.actionType === 'question' ? '質問' : log.actionType === 'guess' ? '解答' : '通知' }} / {{ log.playerName }}</strong>
              <p>{{ log.content }}</p>
            </div>
          </div>
